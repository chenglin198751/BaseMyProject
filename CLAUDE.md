...
如果任务存在不确定性、信息不足、需求歧义或多个可行方案，不要自行猜测用户意图或擅自决策。
应暂停当前操作，明确列出疑问点并向用户确认，待获得答复后再继续执行。
如果需求存在多种合理理解，应列出所有方案供用户选择，禁止通过主观推测代替用户决策。
所有澄清性问题必须在实施前提出。

### 2. Bug 排查遵循证据优先原则

如果问题原因不明确，不要基于猜测直接修改代码。
应先在关键位置增加 Logcat 日志，由用户运行并反馈日志，再根据日志分析问题。
如果仍无法定位根因，应继续补充日志并收集证据，而不是盲目猜测或反复修改代码。
始终遵循：**证据优先，推测靠后。**
